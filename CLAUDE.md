...
- **No Divider needed** — the `Entries` frame uses `itemSpacing: 16` to separate entries; do not add divider rectangles, and individual `Entry` frames carry **no bottom stroke/divider** (`strokes: []`)
...
- **Entry structure** — `Entry/vX.X.X` frame → `Header` → one or more `Section/CATEGORY` frames → each section contains `BulletRow`(s) only (no inter-bullet spacers), plus a `LabelPill` **only when the entry has 2+ sections** (see the *Section LabelPill* rule below). **No `Spacer` layers** — the Header-to-Section and Section-to-Section spacing comes from the Entry frame's `itemSpacing` gap (next rule), never from manual spacer frames.
- **Entry frame spacing (no spacers, no divider)** — every `Entry/vX.X.X` frame uses **`paddingTop = paddingBottom = 12`** and **`itemSpacing = 12`** (left/right padding stay `0`), and **no stroke** (`strokes: []`). This is the single, consistent rhythm for all entries; do not reintroduce `Spacer` frames or a bottom divider. Cloning the most recent entry (per *Style cloning*) already carries these values.
